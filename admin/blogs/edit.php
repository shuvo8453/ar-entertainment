...
/**
 * AR Entertainment - Edit Blog Article
 * 
 * Edit existing article, TinyMCE WYSIWYG, Thumbnail update,
 * SEO suite, and Google SERP preview.
 */
...
try {
    ...
    if (!$article) {
        set_flash('error', 'Article not found or has been deleted.');
        redirect('admin/blogs/index.php');
    }
...
$page_title = 'Edit: ' . truncate_text($article['title'], 40);
$current_page = 'blogs_edit';

// Form Data defaults from DB
$title            = $article['title'];
$slug             = $article['slug'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf()) {
        ...
        try {
            $check_slug = db()->prepare("SELECT COUNT(*) FROM blogs WHERE slug = ? AND id != ?");
            $check_slug->execute([$slug, $article_id]);
        ...
        $thumbnail_path = $thumbnail; // Keep existing by default
        ...
        // If user checked "remove thumbnail"
        if (!empty($_POST['remove_thumbnail']) && empty($_FILES['thumbnail']['name'])) {
            $thumbnail_path = null;
        }
                ...
                <div class="col-12 col-lg-8">
                    ...
                    <div class="card-ar mb-4">
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" id="postTitle" class="form-control form-control-lg bg-dark border-secondary text-white" placeholder="Article Title" value="<?= htmlspecialchars($title) ?>" required autofocus>
                        </div>
                    ...
                    <div class="card-ar mb-4">
                        ...
                        <textarea name="summary" id="postSummary" rows="3" class="form-control bg-dark border-secondary text-white" placeholder="A brief, engaging 2-3 sentence overview of this article..."><?= htmlspecialchars($summary) ?></textarea>
                    </div>
                ...
                <div class="col-12 col-lg-4">
                    ...
                    <div class="card-ar mb-4">
                        <h6 class="fw-bold text-white border-bottom pb-2 mb-3" style="border-color: var(--ar-border-color) !important;">
                            <i class="fa-solid fa-paper-plane text-danger me-2"></i> Update Settings
                        </h6>
                    ...
                    <div class="card-ar mb-4">
                        ...
                        <?php if (!empty($thumbnail)): ?>
                            <div class="mb-3 p-2 rounded" style="background-color: #12131d; border: 1px solid #242638;">
                                <div class="text-muted small mb-1">Current Thumbnail:</div>
                                <img src="<?= upload_url($thumbnail) ?>" alt="Current thumbnail" class="img-fluid rounded mb-2" style="max-height: 140px; width: 100%; object-fit: cover;">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="remove_thumbnail" value="1" id="removeThumb">
                                    <label class="form-check-label text-danger small" for="removeThumb">
                                        <i class="fa-solid fa-trash-can me-1"></i> Remove current image
                                    </label>
                                </div>
                            </div>
                        <?php endif; ?>
