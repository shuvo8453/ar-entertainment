...
/**
 * AR Entertainment - Edit Video Project
 * 
 * Embed parser for YouTube/Vimeo, interactive video preview player,
 * custom thumbnail manager, category picker, and featured toggle.
 */
...
if ($id <= 0) {
    set_flash('danger', 'Invalid video project ID.');
    redirect('admin/portfolio');
}

// Fetch existing video project
try {
    ...
    if (!$project) {
        set_flash('danger', 'Video project not found.');
        redirect('admin/portfolio');
    }
...
$page_title = 'Edit: ' . truncate_text($project['title'], 35);
...
$is_featured   = (int) $project['is_featured'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf()) {
        ...
        try {
            $check_slug = db()->prepare("SELECT COUNT(*) FROM portfolio WHERE slug = ? AND id != ?");
        ...
        if ($remove_thumb && !empty($thumbnail)) {
            $thumb_full_path = UPLOADS_PATH . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $thumbnail);
        ...
        if (!empty($_FILES['thumbnail']['tmp_name'])) {
            ...
            if ($upload_res['success']) {
                ...
                if (!empty($thumbnail)) {
                    $old_path = UPLOADS_PATH . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $thumbnail);
        ...
        if (empty($errors)) {
            try {
                ...
                set_flash('success', "Video project '<strong>" . htmlspecialchars($title) . "</strong>' updated successfully!");
                redirect('admin/portfolio');
            } catch (PDOException $e) {
                $errors[] = 'Failed to update video project: ' . $e->getMessage();
            }
        }
    }
}
...
<div class="admin-main">
    ...
    <main class="admin-content">
            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-1 small text-muted">
                        ...
                        <li class="breadcrumb-item active text-white" aria-current="page">Edit <?= htmlspecialchars(truncate_text($project['title'], 25)) ?></li>
                    </ol>
                </nav>
                <h3 class="fw-bold mb-0 text-white">Edit Video Project</h3>
            </div>
            <div class="d-flex gap-2">
                <a href="<?= site_url('portfolio/' . urlencode($project['slug'])) ?>" target="_blank" class="btn btn-ar-secondary">
        ...
        <form method="POST" action="<?= site_url('admin/portfolio/edit?id=' . $id) ?>" enctype="multipart/form-data" id="projectForm">
            <?= csrf_field() ?>
                ...
                <div class="col-12 col-lg-8">
                    ...
                    <div class="card-ar mb-4">
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" id="projectTitle" class="form-control form-control-lg bg-dark border-secondary text-white" placeholder="e.g. Grameenphone 5G TV Commercial — Future Ready" value="<?= htmlspecialchars($title) ?>" required>
                        </div>
                    ...
                    <div class="card-ar mb-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="video_url" id="videoUrlInput" class="form-control bg-dark border-secondary text-white font-monospace" placeholder="e.g. https://www.youtube.com/watch?v=... OR https://youtu.be/..." value="<?= htmlspecialchars($video_url) ?>" required>
                            <div class="form-text text-muted small">
                                Paste regular YouTube link, Vimeo URL, or copied <code>&lt;iframe&gt;</code> embed code.
                            </div>
                        </div>
                        ...
                        <div id="videoPreviewBox" class="rounded overflow-hidden border border-secondary mt-3" style="background: #000;">
                            ...
                            <div id="noVideoNotice" class="text-center py-5 text-muted" style="display: none;">
                ...
                <div class="col-12 col-lg-4">
                    ...
                    <div class="card-ar mb-4">
                        ...
                        <?php if (!empty($thumbnail)): ?>
                            <div class="mb-3 rounded overflow-hidden p-2 text-center" style="background: #090a0f; border: 1px solid #2a2d40;">
                                <img src="<?= upload_url($thumbnail) ?>" alt="Current thumbnail" class="img-fluid rounded mb-2" style="max-height: 120px;">
                                <div class="form-check text-start ps-4">
                                    ...
                                    <label class="form-check-label text-danger small" for="removeThumbnail">
                                        <i class="fa-solid fa-trash me-1"></i> Remove custom thumbnail (fallback to video platform poster)
                                    </label>
                                </div>
                            </div>
                        <?php endif; ?>
                        ...
                        <div class="mb-3">
                            <label class="form-label text-white small fw-semibold">
                                <?= !empty($thumbnail) ? 'Replace Thumbnail Image' : 'Upload Thumbnail Image' ?>
                            </label>
                        ...
                        <div id="thumbPreviewWrapper" class="text-center rounded p-2" style="background: #090a0f; border: 1px dashed #2a2d40; display: none;">
                            ...
                            <div class="small text-muted mt-1">New Selected Thumbnail</div>
                        </div>
                    </div>
                    ...
                    <div class="card-ar mb-4">
                        ...
                        <div class="d-grid gap-2 pt-2 border-top" style="border-color: var(--ar-border-color) !important;">
                            <button type="submit" class="btn btn-ar-primary btn-lg">
                                <i class="fa-solid fa-floppy-disk me-1"></i> Update Video Project
                            </button>
                            <a href="<?= site_url('admin/portfolio') ?>" class="btn btn-ar-secondary">
                                Cancel
                            </a>
